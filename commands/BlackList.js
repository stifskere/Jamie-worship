...
module.exports = {
    ...
    async execute(interaction, client){
        if(!client.worshipModerators.includes(interaction.user.id)){
            await interaction.reply({content: "You cannot blacklist worshippers, you need to be a JamieWorship moderator."});
            return;
        }
        if(interaction.user.id === interaction.targetUser.id || client.worshipModerators.includes(interaction.targetUser.id)){
            await interaction.reply({content: "You cannot blacklist yourself or any moderator."})
            return;
        }
