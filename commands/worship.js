...
export default {
    ...
    async execute(interaction, client){
        client.db.all(`SELECT id FROM BlackListedUsers WHERE id = '${interaction.user.id}'`, async (err, rows) => {
            if(rows.length !== 0) await interaction.reply({content: 'You are not able to worship, you are in the blacklist, to appeal go to this link <https://rb.gy/enaq3a>', ephemeral: true});
            else {
